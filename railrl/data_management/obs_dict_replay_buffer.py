...
class ObsDictReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        self._action_dim = env.action_space.low.size if env is not None else 2
        ...
        self._next_obs = {}
        a = spaces.Dict({
             'image':spaces.Box(low=np.array([0.0] * 6912), high=np.array([256.0,] * 6912))
        })
        self.ob_spaces = self.env.observation_space.spaces if env is not None else OrderedDict([('image', spaces.Box(0, 256, (6912,)))])
    ...
    def add_path(self, path):
        ...
        actions = noise(flatten_n(actions))
...
def noise(xs):
    return np.random.uniform(-0.01, 0.01, xs.shape) + np.clip(xs, 0.99, -0.99)
