...
class MdpStepCollector(StepCollector):
    def __init__(
        ...
        self._optional_expl_probability = optional_expl_probability
    ...
    def collect_one_step(
        ...
        if self._optional_expl_policy is None:
            action, agent_info = self._policy.get_action(self._obs)
        else:
            if np.random.random() < self._optional_expl_probability:
                action, agent_info = self._optional_expl_policy(self._obs)
            else:
                action, agent_info = self._policy.get_action(self._obs)
